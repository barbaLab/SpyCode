function varargout = multiplerawdataGUI(varargin)
% MULTIPLERAWDATAGUI M-file for multiplerawdataGUI.fig
%      MULTIPLERAWDATAGUI, by itself, creates a new MULTIPLERAWDATAGUI or raises the existing
%      singleton*.
%
%      H = MULTIPLERAWDATAGUI returns the handle to a new MULTIPLERAWDATAGUI or the handle to
%      the existing singleton*.
%
%      MULTIPLERAWDATAGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MULTIPLERAWDATAGUI.M with the given input arguments.
%
%      MULTIPLERAWDATAGUI('Property','Value',...) creates a new MULTIPLERAWDATAGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before multiplerawdataGUI_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to multiplerawdataGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help multiplerawdataGUI

% Last Modified by GUIDE v2.5 28-May-2007 14:36:48

% Begin initialization code - DO NOT EDIT

% Modified by Luca Leonardo Bologna 04 June 2007
%   modified in order to manage up to 64 channels

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @multiplerawdataGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @multiplerawdataGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before multiplerawdataGUI is made visible.
function multiplerawdataGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to multiplerawdataGUI (see VARARGIN)
...
% Choose default command line output for multiplerawdataGUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes multiplerawdataGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = multiplerawdataGUI_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[handles.mcdname handles.mcdpath]=uigetfile('*.mat','Select the MAT file to plot');
...
% handles    structure with handles and user data (see GUIDATA)
handles.mcdname=0;
handles.mcdpath=uigetdir(pwd,'Select the folder which contain MAT files')
[exp_num1]=find_expnum(handles.mcdpath, '_Mat_files');
s=strfind(exp_num1,'_')
exp_num=exp_num1(1:s(1,1)-1)
cd(handles.mcdpath)
cd ..
cd ..

end_fol=strcat(exp_num,'_PlotRawData')
mkdir(end_fol)
a=strfind(handles.mcdpath,'\')
fol=handles.mcdpath(1:a(end-1)-1)
handles.end_folderplot=strcat(fol,'\',exp_num,'_PlotRawData')

if isdir(handles.mcdpath) 
    cd(handles.mcdpath);
    content=dir;
    if length(content)<=2
        msgbox('Directory doesn''t contain any MAT file','WARNING','error');
        handles.mcdpath=0;
    else
...
if (isequal(get(handles.uipanel3,'HighlightColor'),[0.157 0.157 0.157]))||(isequal(get(handles.uipanel6,'HighlightColor'),[0.157 0.157 0.157]))
    
    msgbox('Bad selection','WARNING','error');
...
% % % % %     handles.ch(64)=0;
    ...
    plot_multiplerawdata(handles.ch,handles.mcdpath,handles.end_folderplot);
...
function ch_8_Callback(hObject, eventdata, handles)
if (get(hObject,'Value'))== get(hObject,'Max')
    handles.ch(8)=1;
elseif (get(hObject,'Value'))==get(hObject,'Min')
    handles.ch(8)=0;
...
% --- Executes on button press in ch_57.
function ch_57_Callback(hObject, eventdata, handles)
if (get(hObject,'Value'))== get(hObject,'Max')
    handles.ch(57)=1;
elseif (get(hObject,'Value'))==get(hObject,'Min')
    handles.ch(57)=0;
...
% --- Executes on button press in ch_64.
function ch_64_Callback(hObject, eventdata, handles)
if (get(hObject,'Value'))== get(hObject,'Max')
    handles.ch(64)=1;
elseif (get(hObject,'Value'))==get(hObject,'Min')
    handles.ch(64)=0;
